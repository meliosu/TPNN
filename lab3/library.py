...
def compare_models(results: Dict[str, Dict[str, Any]]) -> None:
    """Compare performance of multiple models with visualizations."""
    # Create figure
    ...
    for model_name, result in results.items():
        plt.plot(result['eval_callback'].batch_numbers, 
                 result['eval_callback'].mse_values, 
                 label=f"{model_name}")
    plt.xlabel('Batch')
    plt.ylabel('MSE')
    ...
    for model_name, result in results.items():
        plt.plot(result['eval_callback'].batch_numbers, 
                 result['eval_callback'].rmse_values, 
                 label=f"{model_name}")
    plt.xlabel('Batch')
    plt.ylabel('RMSE')
    ...
    for model_name, result in results.items():
        plt.plot(result['eval_callback'].batch_numbers, 
                 result['eval_callback'].train_losses, 
                 label=f"{model_name}")
    plt.xlabel('Batch')
    plt.ylabel('Loss')
    ...
    for model_name, result in results.items():
        plt.plot(result['eval_callback'].batch_numbers, 
                 result['eval_callback'].val_losses, 
                 label=f"{model_name}")
    plt.xlabel('Batch')
    plt.ylabel('Loss')
