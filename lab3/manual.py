...
class SimpleRNN:
    ...
    def forward(self, inputs_batch):
        ...
        for b in range(batch_size):
            ...
            # Optimized sequence processing using matrix operations
            for x in inputs:
                x = x.reshape(-1, 1)
    ...
    def backward(self, inputs_batch, all_hidden_states, outputs, targets):
        """Vectorized backward pass handling a batch of sequences"""
        batch_size = len(inputs_batch)
    ...
    def predict(self, inputs):
        """Optimized prediction for a batch of sequences"""
...
class GRU:
    ...
    def backward(self, inputs_batch, all_caches, outputs, targets):
        """Vectorized backward pass for GRU"""
        batch_size = len(inputs_batch)
        ...
        for param, dparam in zip([self.Wz, self.Wr, self.Wh, self.Why, self.bz, self.br, self.bh, self.by],
                                [dWz/batch_size, dWr/batch_size, dWh/batch_size, dWhy/batch_size,
...
class LSTM:
    def __init__(self, input_size, hidden_size, output_size, learning_rate=0.01):
        ...
        # Initialize forget gate bias to 1.0 (important LSTM optimization)
    ...
    def forward(self, inputs_batch):
        ...
        for b in range(batch_size):
            ...
            for x in inputs:
                ...
                c_tilde = tanh(np.dot(self.Wc, xh) + self.bc)
    ...
    def backward(self, inputs_batch, all_caches, outputs, targets):
        """Vectorized backward pass for LSTM"""
        batch_size = len(inputs_batch)

        # Initialize gradients
        dWf, dWi, dWc, dWo, dWhy = np.zeros_like(self.Wf), np.zeros_like(self.Wi), \
                                   np.zeros_like(self.Wc), np.zeros_like(self.Wo), \
                                   np.zeros_like(self.Why)
        dbf, dbi, dbc, dbo, dby = np.zeros_like(self.bf), np.zeros_like(self.bi), \
                                   np.zeros_like(self.bc), np.zeros_like(self.bo), \
        ...
        for param, dparam in zip([self.Wf, self.Wi, self.Wc, self.Wo, self.Why, self.bf, self.bi, self.bc, self.bo, self.by],
                                [dWf/batch_size, dWi/batch_size, dWc/batch_size, dWo/batch_size, dWhy/batch_size,
